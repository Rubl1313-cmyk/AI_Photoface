#!/usr/bin/env python3
"""
🎨 AI PhotoStudio Pro — Ultra Realistic Edition
Main Bot File
"""
import asyncio
import logging
...
from services.enhancement import enhance_image_quality, validate_image_quality
...
async def create_photo_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        "💡 Чем лучше фото, тем реалистичнее результат!"
    )
...
async def simple_generation_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        "💡 Пример: красивый закат над горами, цифровой арт"
    )
...
async def process_about(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        f"🎨 **Что это**:\n"
        f"   Профессиональный бот для создания ultra realistic фото "
        f"с использованием Phoenix 1.0 и FaceFusion.\n\n"
        f"✨ **Возможности**:\n"
        ...
        f"🔗 **Версия**: 2.0.0 Ultra Realistic",
        reply_markup=get_main_menu()
    )
    logger.info(f"🤖 About bot (callback) by user {callback.from_user.id}")

# ------------------------------------------------------------
# ОБРАБОТЧИКИ НИЖНЕГО МЕНЮ (TEXT)
# ------------------------------------------------------------
@dp.message(lambda msg: msg.text and msg.text.strip() == "🔄 С заменой лица")
...
@dp.message(lambda msg: msg.text and msg.text.strip() == "✨ Просто генерация")
...
@dp.message(lambda msg: msg.text and msg.text.strip() == "🖼️ Замена лица на своём изображении")
...
@dp.message(lambda msg: msg.text and msg.text.strip() == "✨ ИИ фотосессия")
...
@dp.message(lambda msg: msg.text and msg.text.strip() == "📊 Моя статистика")
async def process_stats_text(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f"📊 **Ваша статистика**\n"
        f"🔹 Использовано: {used}/{config.DAILY_LIMIT}\n"
        f"🔹 Осталось: {remaining}\n"
        f"🔹 Обновление: завтра в 00:00 UTC"
    )
    logger.info(f"📊 Stats (text) by user {user_id}: {used}/{config.DAILY_LIMIT}")

@dp.message(lambda msg: msg.text and msg.text.strip() == "❓ Помощь")
async def process_help_text(message: types.Message, state: FSMContext):
    await message.answer(
        f"❓ **Помощь — {config.BOT_NAME}**\n\n"
        f"🔄 **С заменой лица**: фото → пол/стиль/кадр → промпт → результат\n"
        f"✨ **Просто генерация**: промпт → изображение\n"
        f"🖼️ **Замена на фото**: 2 фото → замена лица\n"
        f"🎨 **ИИ фотосессия**: фото + промпт → фотосессия\n\n"
        f"📝 Промпт максимум 1024 символа.\n"
        f"📊 Лимит: {config.DAILY_LIMIT}/день"
    )
    logger.info(f"❓ Help (text) by user {message.from_user.id}")

@dp.message(lambda msg: msg.text and msg.text.strip() == "🤖 О боте")
async def process_about_text(message: types.Message, state: FSMContext):
    await message.answer(
        ...
        f"   • 🖼️ Замена лица на фото\n"
        f"   • 📸 ИИ фотосессии\n\n"
        f"🚀 **Технологии**: Phoenix 1.0, FaceFusion, Cloudflare\n"
        f"📊 **Лимит**: {config.DAILY_LIMIT} генераций/день\n"
        f"🔗 **Версия**: 2.0.0 Ultra Realistic"
    )
    logger.info(f"🤖 About bot (text) by user {message.from_user.id}")
...
async def receive_face_photo(message: types.Message, state: FSMContext):
    try:
        ...
        # Валидация качества
        is_valid, error_msg = validate_image_quality(source_image)
        if not is_valid:
...
# ------------------------------------------------------------
# Генерация с заменой лица (ULTRA REALISTIC)
# ------------------------------------------------------------
async def proceed_to_generation(event, state: FSMContext):
    data = await state.get_data()
    ...
    status_msg = await send_message(event, "⏳ Генерирую ultra realistic фото... ~60-120 сек")
    ...
    try:
        ...
        # Формирование ultra realistic промпта
        gender_text = config.GENDER_PROMPTS.get(gender, "")
        shot_text = config.SHOT_PROMPTS.get(shot_type, "")
        style_text = config.STYLE_PROMPTS.get(style, "")
        
        full_prompt = (
            f"{translated}, {gender_text}, {shot_text}, {style_text}, "
            f"{config.ULTRA_REALISTIC_PROMPT}"
        )
        
        logger.info(f"🎨 Ultra Realistic Prompt: {full_prompt[:100]}...")
        
        # Генерация через Phoenix 1.0
        ...
        if isinstance(status_msg, types.CallbackQuery):
            await edit_message(status_msg, "✅ Сгенерировано! Заменяю лицо... ~30-60 сек")
        else:
            await status_msg.edit_text("✅ Сгенерировано! Заменяю лицо... ~30-60 сек")
        ...
        # Финальное улучшение
        result_bytes = enhance_image_quality(result_bytes)
...
# ------------------------------------------------------------
# Простая генерация (PHOENIX)
# ------------------------------------------------------------
async def proceed_simple_generation(event, state: FSMContext):
    ...
    await state.set_state(UserStates.generating)
    status_msg = await send_message(event, "⏳ Генерирую через Phoenix 1.0... ~30-60 сек")
    
    try:
        translator = GoogleTranslator(source='auto', target='en')
        translated = translator.translate(prompt)
        
        full_prompt = f"{translated}, {config.ULTRA_REALISTIC_PROMPT}"
...
# ------------------------------------------------------------
# ИИ фотосессия (ENHANCED)
# ------------------------------------------------------------
async def proceed_photoshoot(event, state: FSMContext):
    data = await state.get_data()
    ...
    await state.set_state(UserStates.generating)
    status_msg = await send_message(event, "🎭 Создаю ultra realistic фотосессию... ~60-90 сек")
    ...
    try:
        ...
        enhanced_prompt = (
            f"{translated}, {gender_text}, {shot_text}, "
            f"{config.ULTRA_REALISTIC_PROMPT}, "
            f"professional photo shoot, looking into the camera"
        )
        ...
        caption = truncate_caption(f"✅ Ultra Realistic фотосессия готова!\n🎨 Промпт: {prompt}")
...
async def receive_target_for_swap(message: types.Message, state: FSMContext):
    try:
        ...
        # Валидация
        is_valid, error_msg = validate_image_quality(target_image)
        if not is_valid:
            await message.answer(error_msg)
            await state.clear()
            return
        ...
        try:
            ...
            caption = truncate_caption("✅ Ultra Realistic! Лицо заменено. 🎭")
    ...
    try:
        ...
        try:
            await bot.send_message(
                config.ADMIN_ID,
                f"🚀 {config.BOT_NAME} v2.0 Ultra Realistic запущен! ({datetime.now().strftime('%Y-%m-%d %H:%M')})"
            )
        except:
            pass
