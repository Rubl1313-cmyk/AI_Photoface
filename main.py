...
# 🎯 MAIN — ИСПРАВЛЕНО: AUTO-SET WEBHOOK
async def main():
    global face_swapper
    if not check_config(): sys.exit(1)
    face_swapper = FaceFusionClient(api_url=FACEFUSION_URL)
    
    # 🔥 АВТОМАТИЧЕСКАЯ НАСТРОЙКА WEBHOOK (вместо delete!)
    try:
        # Получаем URL Render из переменной окружения или используем дефолтный
        render_url = os.getenv("RENDER_EXTERNAL_URL", "https://ai-photoface.onrender.com").rstrip("/")
