...
✅ Polling + Health server на порту 8080 (требование Render)
✅ user_ dict = {} (ПРАВИЛЬНЫЙ СИНТАКСИС)
✅ --output-path для FaceFusion
✅ Гендер в промптах
✅ full_prompt в Cloudflare
"""
...
async def retry_gen(cb: types.CallbackQuery, state: FSMContext):
    ...
    if not data.get('prompt') or uid not in user_data or not user_data[uid].get('face'):
        await cb.message.answer("❌ Начни сначала", parse_mode="Markdown"); await go_home(cb, state); return
...
async def run_health_server():
    """Простой HTTP сервер на порту 8080 для Render.com"""
    async def health_handler(request):
        return web.json_response({"status": "ok", "service": "ai-photo-studio"})
    async def root_handler(request):
        return web.json_response({"service": "AI PhotoStudio", "mode": "polling"})
    ...
    logger.info(f"🌐 Health server running on 0.0.0.0:8080")
...
async def main():
    ...
    logger.info(f"🚀 {BOT_NAME} | Polling + Health server (Render.com)")
    
    # 🔥 Запускаем health server И polling параллельно
    await asyncio.gather(
        run_health_server(),  # ✅ Слушает порт 8080 для Render
        run_polling()          # ✅ Polling для Telegram
    )
